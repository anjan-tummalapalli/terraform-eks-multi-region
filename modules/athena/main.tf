...
# Resource Purpose: Configures an Athena workgroup for query execution
# settings, output location, and scan controls (aws_athena_workgroup.this).
resource "aws_athena_workgroup" "this" {
  ...
  configuration {
    ...
    # Dynamic Purpose: Adds an engine_version block only when an explicit
    # Athena engine version is provided.
    ...
    result_configuration {
      ...
      encryption_configuration {
        ...
        # Ternary Purpose: Selects the "kms_key_arn" value by evaluating a
        # condition and choosing true/false branches explicitly.
        kms_key_arn = (
          var.result_encryption_option == "SSE_KMS"
          ? var.result_kms_key_arn
          : null
        )
      }
    }
...
# Resource Purpose: Creates an Athena database container for table metadata and
# query namespaces (aws_athena_database.this).
resource "aws_athena_database" "this" {
  ...
  # Dynamic Purpose: Adds database encryption settings only when database-level
