...
# Purpose:
#   Declares input interface for module 'sns-topic' (types, defaults,
# validation).
...
# Variable Purpose: Whether Simple Notification Service (SNS) topic is First
# In, First Out (FIFO).
variable "fifo_topic" {
...
# Variable Purpose: Optional Key Management Service (KMS) key ID for topic
# encryption.
