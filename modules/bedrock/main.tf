...
locals {
  # Local Purpose: Defines derived value "enable_s3_logging" once for reuse and
  # consistent logic across this file.
  enable_s3_logging = (
    var.s3_bucket_name != null && trim(var.s3_bucket_name) != ""
  )

  # Local Purpose: Defines derived value "effective_log_group_name" once for
  # reuse and consistent logic across this file.
  effective_log_group_name = (
    coalesce(
      var.cloudwatch_log_group_name,
      "/aws/bedrock/model-invocations/${var.name_prefix}"
    )
  )
  ...
  effective_role_arn = var.enable_cloudwatch_logging ? (
    ...
    # expression branches.
    var.create_logging_role
    ? aws_iam_role.bedrock_logging[0].arn
    : var.logging_role_arn
  ) : null

  # Local Purpose: Defines derived value "cloudwatch_log_group_arn" once for
  # reuse and consistent logic across this file.
  cloudwatch_log_group_arn = (
    format(
      ...
      local.effective_log_group_name
    )
  )
}
...
# Data Purpose: Reads data source aws_iam_policy_document.logging to fetch
# existing Amazon Web Services (AWS) context required by dependent expressions.
data "aws_iam_policy_document" "logging" {
  ...
  # Dynamic Purpose: Adds an Identity and Access Management (IAM) statement for
  # Simple Storage Service (S3) delivery only when Bedrock log delivery to
  # Simple Storage Service (S3) is enabled.
  dynamic "statement" {
...
resource "aws_cloudwatch_log_group" "this" {
  ...
  count = (
    var.enable_cloudwatch_logging && var.create_cloudwatch_log_group ? 1 : 0
  )
...
resource "aws_iam_role" "bedrock_logging" {
  ...
  name = (
    coalesce(var.logging_role_name, "${var.name_prefix}-bedrock-logging-role")
  )
...
# policy document directly to a role (aws_iam_role_policy.bedrock_logging).
...
# Resource Purpose: Configures Bedrock model invocation logging destinations
# and payload delivery options
...
resource "aws_bedrock_model_invocation_logging_configuration" "this" {
  logging_config {
    ...
    # Dynamic Purpose: Adds CloudWatch logging destination settings only when
    # CloudWatch delivery is enabled.
    dynamic "cloudwatch_config" {
      ...
      content {
        ...
        # Dynamic Purpose: Adds Simple Storage Service (S3) delivery
        # configuration for large payload logs only when Simple Storage Service
        # (S3) logging is enabled.
        dynamic "large_data_delivery_s3_config" {
          # Ternary Purpose: Selects the "for_each" value by evaluating a
    ...
    # Dynamic Purpose: Adds primary Simple Storage Service (S3) logging
    # destination settings only when Simple Storage Service (S3) delivery is
    # enabled.
    dynamic "s3_config" {
      # Ternary Purpose: Selects the "for_each" value by evaluating a condition
      # and choosing true/false branches explicitly.
  ...
  lifecycle {
    precondition {
      ...
      error_message = <<-EOT
        Enable at least one Bedrock logging destination: CloudWatch or S3.
      EOT
    }
    ...
    precondition {
      ...
      error_message = <<-EOT
        Set logging_role_arn when enable_cloudwatch_logging is true and
        create_logging_role is false.
      EOT
    }
    ...
    precondition {
      ...
      error_message = <<-EOT
        At least one Bedrock payload type must be enabled for delivery.
      EOT
    }
  }
}
